...
// Think of it like a notebook where a trader writes down WHY they are making a trade,
...
// the new owner must explicitly accept ownership so you cannot accidentally hand control to a wrong address.
...
// calls back into your function before it is done executing (like someone sneaking back into
...
// like a circuit breaker in your house that trips when there is a power surge.
...
// Thrown when someone references a thesis ID that does not exist — like looking up a book by a call number that was never assigned.
error ThesisEngine__InvalidThesisId();

// Thrown when someone tries to modify a thesis that has already been closed — like trying to edit a sealed envelope.
...
contract ThesisEngine is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    // NEUTRAL = "I am not betting on direction" (like a hedge or a market-neutral strategy)
    ...
    struct DCFParams {
        ...
        // Like saying "money today is worth more than money tomorrow, and here is by how much."
        ...
        // How many years into the future we are projecting cash flows.
        ...
        // Like the listing price of a house on Zillow vs. what the appraiser says it is worth.
        uint256 currentPriceUSD;

        // The margin of safety in basis points — how much cheaper the current price is vs. fair value.
        // Like buying a house that is appraised at 500K for only 350K — the 30% discount is your safety cushion
    ...
    struct Thesis {
        ...
        // Like listing which stocks you are watching for this particular trade idea.
        string[] tickers;
        ...
        // Like setting a goal: "I will sell when it hits this price."
        uint256 targetPriceUSD;

        // The price at which the trader will cut losses — their safety net.
        // Like saying "if it drops to HERE, I am out" to limit damage.
        uint256 stopLossUSD;

        // How many days the trader plans to hold the position.
        // Like a countdown timer on the trade — "I will give this idea 30 days to play out."
        ...
        // Like writing "here is what I learned" at the end of an experiment.
        string closingNotes;
    }
    ...
    // A mapping from trader address to an array of thesis IDs they have created.
    ...
    // whenNotPaused = only works if the emergency stop has not been pulled.
    ...
    function createThesis(
        ...
        // If no tickers were provided, revert — you cannot have a trade idea about nothing.
        ...
        // Like tearing off a ticket number — you take the current one, and the next person gets the next.
        thesisId = _nextThesisId++;

        // Get a storage pointer to the thesis slot in the mapping.
        // "storage" means we are writing directly to the blockchain's permanent memory,
        ...
        // Record the entry price — the price at which we are getting in.
        t.entryPriceUSD = _entryPriceUSD;

        // Record the target price — the price at which we would take profit.
        t.targetPriceUSD = _targetPriceUSD;

        // Record the stop-loss price — the price at which we would cut our losses.
    ...
    function closeThesis(uint256 _thesisId, ThesisStatus _status, string calldata _notes) external nonReentrant {
        ...
        // If not, revert — you cannot close someone else's trade journal entry.
        if (t.trader != msg.sender) revert ThesisEngine__NotThesisOwner();

        // Check that the thesis is still ACTIVE — you cannot close something that is already closed.
        // Like trying to seal an envelope that is already sealed.

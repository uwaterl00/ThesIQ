...
contract ThesisEngine is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    function createThesis(
        CreateThesisParams memory _params
    ) external nonReentrant whenNotPaused returns (uint256 thesisId) {
        if (_params.tickers.length == 0) revert ThesisEngine__EmptyTicker();
        if (_params.tickers.length > MAX_TICKERS_PER_THESIS) revert ThesisEngine__MaxTickersExceeded();
        if (_params.holdPeriodDays == 0 || _params.holdPeriodDays > 56) revert ThesisEngine__InvalidTimeframe();
        ...
        t.id = thesisId;
        t.trader = msg.sender;
        t.tickers = _params.tickers;
        t.direction = _params.direction;
        t.status = ThesisStatus.ACTIVE;
        t.entryPriceUSD = _params.entryPriceUSD;
        t.targetPriceUSD = _params.targetPriceUSD;
        t.stopLossUSD = _params.stopLossUSD;
        t.holdPeriodDays = _params.holdPeriodDays;
        t.convictionScore = _params.convictionScore;
        t.catalystHash = _params.catalystHash;
        t.dcf = _params.dcf;
        t.createdAt = block.timestamp;

        traderTheses[msg.sender].push(thesisId);

        emit ThesisCreated(thesisId, msg.sender, _params.direction, _params.convictionScore);
    }
